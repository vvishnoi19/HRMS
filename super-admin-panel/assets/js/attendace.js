    ...
    document.querySelectorAll('[data-main-tab]').forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            // Agar requests hai to sub-tabs banao
            if (tabConfigs[mainTab]) {
                updateSubTabs(mainTab);
                const firstSubTab = tabConfigs[mainTab].tabs[0].target;
                showContent(firstSubTab);
                attachSubTabListeners();
            } else {
                // Agar overview hai to directly overview content show karo
                document.getElementById('sub-tabs').innerHTML = ''; // empty sub-tabs
                showContent('overview-my-team'); // ✅ overview ka default content id
            }
        });
    });

    // ---- DEFAULT LOAD: Overview tab active ----
    const defaultTab = document.querySelector('[data-main-tab="overview"]');
    if (defaultTab) {
        defaultTab.classList.add('active'); // tab highlight
        document.getElementById('sub-tabs').innerHTML = ''; // no sub-tabs
        showContent('overview-my-team'); // ✅ first load me overview content show karo
    }
}
